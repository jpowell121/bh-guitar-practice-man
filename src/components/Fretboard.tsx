...
import { FRETBOARD } from "@/data/standardTuning";
import { generateScale, ScaleType, Enharmonic } from "@/lib/scaleHelpers";
...
const STRING_THICKNESSES = [1.2, 1.5, 1.8, 2.0, 2.6, 3.0];
...
export default function Fretboard({
    ...
    const regNoteId = `regularNoteGrad-${wood}`;
        ...
        >
            <svg viewBox={`0 0 ${SVG_WIDTH} ${MARGIN_TOP + boardHeightActual + MARGIN_BOTTOM}`}>
                <defs>
                    ...
                    <radialGradient id="noteGlow" cx="35%" cy="35%" r="60%">
                        <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.6" />
                        <stop offset="100%" stopColor="#000000" stopOpacity="0" />
                    </radialGradient>
                    <radialGradient id="rootNoteGrad" cx="35%" cy="35%" r="60%">
                        <stop offset="0%" stopColor="#5BBFBF" />
                        <stop offset="100%" stopColor="#1A6B6B" />
                    </radialGradient>
                    <radialGradient id={regNoteId} cx="35%" cy="35%" r="60%">
                        <stop offset="0%" stopColor={cfg.noteRegularFrom} />
                        <stop offset="100%" stopColor={cfg.noteRegularTo} />
                    </radialGradient>
                </defs>
                ...
                {Array.from({ length: NUM_STRINGS }, (_, si) => si + 1).flatMap((s) =>
                    Array.from({ length: clampedFrets }, (_, fi) => {
                        ...
                        const x = stringX(s);
                        const y = fretY(fi) + fretSpacing / 2;
                        const r = 14;
                        return (
                            <g key={`note-${s}-${fret}`}>
